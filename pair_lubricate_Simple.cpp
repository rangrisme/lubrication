...
/*
	...
	double hinv,lhinv,XA11=0,YA11=0,YB11=0,YC12=0,ws[3],wd[3],nx,ny,nz;
	...
	beta1=1.0+beta0;
        ...
        if (flaglog) {
		XA11=6*MY_PI*mu*radi*( hinv*2.0*pow(beta0,2) + lhinv*beta0*(1+ 7*beta0+ beta0*beta0)/5.0 )/pow(beta1,3);
		YA11=1.6*MY_PI*mu*radi*lhinv*beta0*(2.0+beta0+2*beta0*beta0)/pow(beta1,3);
		YB11=-0.8*MY_PI*mu*radi*radi*lhinv*beta0*(4+beta0)/(beta1*beta1);
		YC12=0.8*MY_PI*mu*pow(radi,3)*lhinv*beta0*beta0/beta1*(1.0-4.0/beta0); //YC12*(1-4/beta0)
        } 
	else  XA11=12*MY_PI*mu*radi*hinv*pow(beta0,2)/pow(beta1,3);
	...
	//f=XA11 nn dot vr
        ...
        if (flaglog) {
		double ybfac=(1.0-beta0*(1+4*beta0)/(4+beta0))*YB11;
